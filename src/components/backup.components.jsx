...
export function BackupDropdown({ confirmRef }) {
  ...
  return (
    <NavDropdown title={<CloudArrowUpFill width='24' height='24' title={t('header.backup.backup')} style={{ color: 'var(--bs-blue)' }} />} id='backup' align='end' className='px-2 py-2 fw-bolder'>
      ...
      <NavDropdown.Item href='#backup-daily' title={t('header.backup.daily')} active={backup.autoBackup === AUTO_BACKUP.DAILY} onClick={() => onBackup(AUTO_BACKUP.DAILY)}>
        {t('header.backup.daily')}
      </NavDropdown.Item>
      <NavDropdown.Item href='#backup-weekly' title={t('header.backup.weekly')} active={backup.autoBackup === AUTO_BACKUP.WEEKLY} onClick={() => onBackup(AUTO_BACKUP.WEEKLY)}>
        {t('header.backup.weekly')}
      </NavDropdown.Item>
      <NavDropdown.Item href='#backup-monthly' title={t('header.backup.monthly')} active={backup.autoBackup === AUTO_BACKUP.MONTHLY} onClick={() => onBackup(AUTO_BACKUP.MONTHLY)}>
        {t('header.backup.monthly')}
      </NavDropdown.Item>
      <NavDropdown.Item href='#backup-off' title={t('header.backup.off')} active={!backup.autoBackup || backup.autoBackup === AUTO_BACKUP.OFF} onClick={() => onBackup(AUTO_BACKUP.OFF)}>
        {t('header.backup.off')}
      </NavDropdown.Item>
      <NavDropdown.Divider />
