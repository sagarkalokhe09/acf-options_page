...
import { ElementUtil } from "@dhruv-techapps/core-components";
...
const Config = ({ configs, selected, setConfigs }) => {
  ...
  const onChange = (e) => {
    const { name, value } = ElementUtil.getNameValue(e.currentTarget, config)
  ...
  return <>
        ...
        <Card.Header as="h5">
          <Row>
            ...
            <Col md="auto" className="d-flex align-items-center">
              <Form.Check type="switch" name="enable" id="config-enable" is={typeof config.enable} label="Enable" checked={config.enable} onChange={onChange} />
          ...
          <Row>
            ...
            <Col md="5" sm="12">
              <InputGroup className="mb-3">
                ...
                <FormControl value={config.initWait} name="initWait" onChange={onChange} placeholder="0" aria-label="0" aria-describedby="config-init-wait" />
              </InputGroup>
            </Col>
            <Col md="7" sm="12">
              <InputGroup className="mb-3">
                ...
                <FormControl value={config.startTime} typeof={typeof config.startTime} name="startTime" onChange={onChange} placeholder="hh:mm:ss" aria-label="hh:mm:ss" aria-describedby="config-start-time" />
              </InputGroup>
            </Col>
          </Row>
        </Card.Body>
    ...
    <Batch batch={config.batch} setConfigs={setConfigs} />
